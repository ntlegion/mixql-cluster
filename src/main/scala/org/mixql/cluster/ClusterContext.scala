...
class ClusterContext(
...
) extends Context(engines, defaultEngine, variables, functions)
  ...
  override def close() = {
    ...
    broker.close()
    println(
      "Cluster context: stop engines, if they were not closed before by shutdown command"
    )
    engines.values.foreach(engine => {
      if engine.isInstanceOf[ClientModule] then
        val remoteEngineClient: ClientModule = engine.asInstanceOf[ClientModule]
        println(
          s"Cluster context: stopping remote engine " + remoteEngineClient.name
        )
